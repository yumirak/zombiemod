...
{
	register_native("zb3_set_head_attachment", "Native_ShowAttachment", 1)
}
...
public Show_Attachment(id, const Sprite[],  Float:Time, Float:Scale, Float:FrameRate, TotalFrame)
{
	...
	ducking = (pev(id, pev_flags) & FL_DUCKING)
	Origin[2] += ducking ? 20.0 : 30.0
...
public fw_Think(Ent)
{
	...
	ducking = (pev(Owner, pev_flags) & FL_DUCKING)
	Origin[2] += ducking ? 20.0 : 30.0
